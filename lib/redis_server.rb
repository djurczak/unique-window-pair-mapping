  ...
  def perform(path_to_windows, path_to_anchors, path_to_results)
    ...
    files.each_with_index do |file, i|
      thread_pool.process {
        ...
        cmdline = "ruby #{File.expand_path(File.dirname(__FILE__))}/run_redis_client.rb redis_host=localhost redis_port=#{redis_port} path_to_file=#{file} mode=write orientation=#{orientation}"
        puts cmdline
